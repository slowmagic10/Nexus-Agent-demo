...
export class TerminalUI {
  ...
  render(state) {
    ...
    const streamingAnswer = (state.modelStreamChunks || []).join("");
    console.log(`\n${bold}最近回答${reset}\n${streamingAnswer || this.lastAnswer}`);
    console.log(`\n${dim}/help 帮助  /state 状态  /events 事件  /evaluation 诊断  /memory 会话记忆  /long-memory 长期记忆  /pin 固定记忆  /unpin 取消固定  /memory-issues 异常写入  /sessions 会话  /export 导出  /quit 退出${reset}`);
  }
